...
function M.replace_character()
  ...
  -- Only proceed if we're at a valid position
  if #line >= col + 1 then
...
function M.setup(opts)
  ...
  -- dw to replace numbers with dashes horizontally
  vim.keymap.set('n', 'dw', M.replace_numbers_with_dashes, {
    desc = "Replace numbers with dashes horizontally (in tab mode)",
    silent = true
  })
  
  -- Shift+R to replace character without moving cursor
  vim.keymap.set('n', 'R', M.replace_character, {
    desc = "Replace character and stay in place (in tab mode)",
    silent = true
  })
end
