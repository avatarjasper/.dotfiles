...
function M.setup(opts)
  ...
  for i = 0, 9 do
    vim.keymap.set('n', tostring(i), function() M.replace_with_char(tostring(i)) end, {
      desc = string.format("Replace dash with number %d (in tab mode)", i),
      silent = true
    })
  end
  
  -- Special characters to replace dashes
  local special_chars = {
    s = 's',
    ['/'] = '/',
    ['\\'] = '\\',
  ...
  for key, char in pairs(special_chars) do
    vim.keymap.set('n', key, function() M.replace_with_char(char) end, {
      desc = string.format("Replace dash with '%s' (in tab mode)", char),
      silent = true
    })
  end
end
